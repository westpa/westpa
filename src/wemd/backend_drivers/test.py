...
class TestBackend(BackendDriver):
    ...
    def update_progress_coordinate(self,segment):
        ...
        #extend pcoord by 1
        pcoord = numpy.empty((segment.pcoord.shape[0]+1,segment.pcoord.shape[1]),dtype=numpy.float64)
      
        for i in xrange(0, len(segment.pcoord)):
            pcoord[i] = segment.pcoord[i]

        #discrete overdamped Langevin equation
        #x_{j+1} = x_j - {delta t} over {m %gamma} (dU/dx)_xj + delta{x^rand}
        pcoord[-1] = segment.pcoord[-1] - tmg * self.diff_potential( segment.pcoord[-1], dim ) / nano + numpy.random.normal(0, std, (1,dim)) / nano
