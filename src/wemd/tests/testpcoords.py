...
class TestParticleSet:    
    def test_get_all_bins(self):
        pset = wemd.pcoords.ParticleSet()
        pset.add(wemd.Segment(weight=0.1))
        pset.add(wemd.Segment(weight=0.1))        
...
class TestParticleList:
    def test_clear(self):
        plist = wemd.pcoords.ParticleList()
        plist.append(wemd.Segment(weight=0.1))
        plist.append(wemd.Segment(weight=0.1))
        plist.clear()
        assert len(plist) == 0
...
class TestRectilinearRegionSet:
    def test_1d_index_mapping(self):
        ...
        rrs = wemd.pcoords.RectilinearRegionSet(boundaries)
...
class TestRegionSet:
    def test_replace_region(self):
        ...
        rrso = wemd.pcoords.RectilinearRegionSet(boundaries_outer)
        rrsi = wemd.pcoords.RectilinearRegionSet(boundaries_inner)
