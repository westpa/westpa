...
class WEMDDataReaderMixin(AnalysisMixin):
    ...
    def add_args(self, parser, upcall = True):
        ...
        group.add_argument('-W', '--wemd-data', dest='wemd_h5name', metavar='WEMD_H5FILE',
    ...
    def process_args(self, args, upcall = True):            
        ...
        self.data_manager.backing_file = self.wemd_h5name        
...
class ExtDataReaderMixin(AnalysisMixin):
    ...
    def process_args(self, args, upcall = True):            
        ...
        if args.usecols:
            wemd.rc.pstatus('Using only the following columns from external input: {!s}'.format(args.usecols))
    ...
    def text_to_h5dataset(self, fileobj, group, dsname, dtype=numpy.float64, 
        ...
        for line in fileobj:
            ...
            if irow == chunksize:
                wemd.rc.pstatus('\r  Read {:d} rows'.format(nrows), end='')
                wemd.rc.pflush()
        ...
        wemd.rc.pstatus('\r  Read {:d} rows'.format(nrows))
        wemd.rc.pflush()
    ...
    def npy_to_h5dataset(self, array, group, dsname, usecols=None, chunksize=None):
        ...
        if usecols:
            for istart in xrange(0,maxlen,chunksize):
                ...
                wemd.rc.pstatus('\r  Read {:{mw}d}/{:>{mw}d} rows'.format(iend,maxlen, mw=mw), end='')
                wemd.rc.pflush()
        else:
            for istart in xrange(0,maxlen,chunksize):
...
class BFDataManager(AnalysisMixin):
    ...
    def process_args(self, args, upcall = True):
        ...
        wemd.rc.pstatus("Using brute force data from '{}'".format(self.bf_h5name))
