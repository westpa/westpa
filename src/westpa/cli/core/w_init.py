    ...
    with work_manager:
        if work_manager.is_master:
            ...
            if abs(1.0 - tprob) > len(basis_states) * EPS:
                pscale = 1 / tprob
                log.warning('Basis state probabilities do not add to unity (basis: {:.2f}, start states: {:.2f}); rescaling by {:g}. If using start-states, some rescaling is normal.'.format(bstate_prob, sstate_prob, pscale))
