...
class HDF5MDTrajectory(Trajectory):
    ...
    def __init__(self, atom_indices=None):
        def fget(walker, include_initpoint=False, atom_indices=atom_indices):
            ...
            with WESTIterationFile(traj_filename) as traj_file:
                traj = traj_file.read_as_traj(iteration=iter.number, segment=walker.index, atom_indices=atom_indices)
