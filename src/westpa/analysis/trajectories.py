...
class HDF5MDTrajectory(Trajectory):
    """A MD trajectory stored by the HDF5 framework.
    ...
    def __init__(self):
        def fget(walker, include_initpoint=False, atom_indices=None):
            from westpa.core.h5io import WESTIterationFile

            iteration = walker.iteration
            if 'trajectories' not in iteration.h5group:
                raise ValueError("The loaded west.h5 does not seem to be generated by the HDF5 framework")
            link = iteration.h5group['trajectories']
            traj_filename = link.file.filename

            with WESTIterationFile(traj_filename) as traj_file:
                traj = traj_file.read_as_traj(iteration=iteration.number, segment=walker.index, atom_indices=atom_indices)
            ...
            if include_initpoint:
                ...
                if isinstance(parent, InitialState):
                    ...
                    iter0 = Iteration(0, iteration.run)
