...
class WESTSystem:
    ...
    pcoord_len : int, default 2
        ...
        including *both* the initial and final values. Defaults to 2 (i.e. only the
        initial and final progress coordinate values for a segment are returned from
        propagation).
    pcoord_dtype : type, default numpy.float32
        Scalar type of the progress coordinate.
    bin_mapper : BinMapper, default NopMapper()
        Bin mapper for partitioning progress coordinate space.
    bin_target_counts : ArrayLike, default array([1])
        Vector of target counts, one per bin.
    sink : Sink, optional
        Subset of progress coordinate space to use as the sink. If specified, this
        overrides the sink defined by `bin_mapper` and any target states.
