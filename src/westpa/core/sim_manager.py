...
class WESimManager:
    ...
    def propagate(self):
        ...
        while segment_futures:
            # TODO: add capacity for timeout or SIGINT here
            future = self.work_manager.wait_any(segment_futures)
        ...
        # Assign bins. This is done only after all segments have completed to
        # accommodate "global" binning schemes (e.g., MAB) that require all the
        # segments at the same time.
        self.we_driver.assign(self.segments.values())

        # Construct initial states. This must be done after assigning bins, since
        # `we_driver.n_istates_needed` depends on `we_driver.final_binning`.
        istate_gen_futures = self.get_istate_futures()
        while istate_gen_futures:
            future = self.work_manager.wait_any(istate_gen_futures)
