...
from westtools.files import load_npy_or_text

import logging
log = logging.getLogger('w_ttimes_bf')

from westtools.aframe import WESTAnalysisTool,BinningMixin,MCBSMixin,TransitionAnalysisMixin,KineticsAnalysisMixin
from westtools.aframe.transitions import TransitionEventAccumulator
...
class WTTimesBF(KineticsAnalysisMixin,MCBSMixin,TransitionAnalysisMixin,BinningMixin,WESTAnalysisTool):
    def __init__(self):
    ...
    def assign_to_bins(self):
        ...
        for (ifile, filename) in enumerate(self.input_files):
            ...
            for istart in xrange(0,nrows,self.chunksize):
                ...
                west.rc.pstatus('\r  {:s}: {:{mlnr}d}/{:<{mlnr}d} = {:.{pp}f}%'.format(filename, istart+len(assignments), nrows,
    ...
    def find_transitions(self):
        ...
        for istart in xrange(0, nrows, chunksize):
            ...
            west.rc.pstatus('\r  {:{mwnr}d}/{:<{mwnr}d} ({:.2f}%)'.format(istart+len(assignments), nrows, 
...
if args.usecols:
    west.rc.pstatus('Using only the following columns from the input file: {!s}'.format(args.usecols))
