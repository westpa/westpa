...
from wemdext.stringmethod import WEMDStringMethod, DefaultStringMethod
from wemd.pcoords import VoronoiRegionSet
...
class StringDriver(object):
    def __init__(self, sim_manager):
        ...
        self.windowsize = wemd.rc.config.get_int('stringmethod.windowsize', 10)
        self.update_interval = wemd.rc.config.get_int('stringmethod.update_interval', 10)
        self.initial_update = wemd.rc.config.get_int('stringmethod.initial_update', 20)
        self.priority = wemd.rc.config.get_int('stringmethod.priority', 0)

        self.write_avg_pos = wemd.rc.config.get_bool('stringmethod.write_avgpos', True)
        self.do_update = wemd.rc.config.get_bool('stringmethod.do_update', True)
        self.init_from_data = wemd.rc.config.get_bool('stringmethod.init_from_data', True)
        ...
        wemd.rc.pstatus('-wemdext.stringmethod -----------------\n')
        wemd.rc.pstatus('windowsize: {}\n'.format(self.windowsize))
        wemd.rc.pstatus('update interval: {}\n'.format(self.update_interval))
        wemd.rc.pstatus('initial update: {}\n'.format(self.initial_update))
        wemd.rc.pstatus('priority: {}\n'.format(self.priority))
        wemd.rc.pstatus('write average positions: {}\n'.format(self.write_avg_pos))
        wemd.rc.pstatus('do update: {}\n'.format(self.do_update))
        wemd.rc.pstatus('initialize from WE data: {}\n'.format(self.init_from_data))
        wemd.rc.pstatus('----------------------------------------\n') 
        wemd.rc.pflush()
    ...
    def get_dfunc_method(self):
        methodname = wemd.rc.config.require('stringmethod.dfunc_method')
        pathinfo = wemd.rc.config.get_pathlist('stringmethod.module_path', default=None)
    ...
    def get_avgpos_method(self):
        methodname = wemd.rc.config.require('stringmethod.avgpos_method')
    ...
    def get_string_method(self):
        methodname = wemd.rc.config.require('stringmethod.string_method')
        ...
        assert issubclass(str_method, WEMDStringMethod)
    ...
    def update_regionset(self):
        ...
        wemd.rc.pstatus('wemdext.stringmethod: Updating bin definitions\n')
        wemd.rc.pflush()
    ...
    def prepare_new_segments(self):
        ...
        wemd.rc.pstatus('-wemdext.stringmethod -----------------\n')
        wemd.rc.pstatus('wemdext.stringmethod: Calculating average position in string images\n')
        wemd.rc.pflush()
        ...
        wemd.rc.pstatus('wemdext.stringmethod: Updating string\n')
        wemd.rc.pflush()

        self.strings.update_string_centers(avg_pos,sum_bin_weight)

        wemd.rc.pstatus('wemdext.stringmethod: String lengths: {}\n'.format(self.strings.length))
        wemd.rc.pflush()
