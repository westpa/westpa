...
Instantaneous, average, or evolution of PDFs may be plotted.
'''
    ...
    def _add_plot_options(self, parser):
        ...
        pgroup.add_argument('--range',
                            ...
                            this is the Y axis. For 2-D plots, this is the colorbar axis. This is the
                            output range (i.e. range of logs for log plots). 
                            (Default: full range.)''')
        ...
        for subparser in (iparser, aparser, eparser):
            ...
            if subparser is not eparser:
                ogroup.add_argument('--text-output',
                                    help='''Store plot data in a text format at TEXT_OUTPUT (default: no
                        text output).''')
    ...
    def do_evolution_plot(self):
        ...
        binbounds = self.input_h5['binbounds_{}'.format(idim)][...]
        midpoints = self.input_h5['midpoints_{}'.format(idim)][...]
        hists = self.input_h5['histograms'][iiter_start:iiter_stop]
        
        for ihist in xrange(len(hists)):
            # Average over other dimensions
            for dim in xrange(hists[ihist].ndim):
                if dim != idim:
                    hists[ihist] = numpy.add.reduce(hists[ihist], axis=dim)    
            normhistnd(hists[ihist], [binbounds])
            
        enehists = -numpy.log(hists)
        log10hists = numpy.log10(hists)
        ...
        if self.hdf5_output_filename:
            with h5py.File(self.hdf5_output_filename, 'w') as output_h5:
                ...
                output_h5.attrs['source_dimension'] = idim
                output_h5['midpoints'] = midpoints
                output_h5['histograms'] = hists
                output_h5['n_iter'] = n_iters[iiter_start:iiter_stop]
        ...
        if self.plot_output_filename:
            ...
            pyplot.imshow(plothist, interpolation='nearest', aspect='auto',
                          extent=(midpoints[0], midpoints[-1], n_iters[iiter_start], n_iters[iiter_stop-1]),
                          origin='lower', vmin=vmin, vmax=vmax)
            cb = pyplot.colorbar()
            cb.set_label(label)
            pyplot.xlabel(self.dimensions[0]['label'])
