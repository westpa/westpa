...
ds_uncorrel = ds_correl = numpy.loadtxt('/home/mzwier/flexible-p53-MDM2-1d-rmsd/avgflux_0_blocked.txt', usecols=[2])
...
k_uncorrel = mcbs_correltime(ds_uncorrel,0.05)
k_correl = mcbs_correltime(ds_correl,0.05)
print('  uncorrelated:    {}'.format(k_uncorrel))
print('  correlated:      {}'.format(k_correl))


print('naive MCBS:')
uncorrel_mean, uncorrel_lb, uncorrel_ub = mcbs_ci(ds_uncorrel, numpy.mean, 0.05, n_sets=100000, sort=numpy.msort)
correl_mean, correl_lb, correl_ub = mcbs_ci(ds_correl, numpy.mean, 0.05, n_sets=100000, sort=numpy.msort)
