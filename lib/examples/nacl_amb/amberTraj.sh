...
# Now, the file listing all of the successful iterations and segments from
# the simulation is read line by line and the iteration IDs and segment IDs
...
# that was created above as input.  The text normally displayed to the terminal 
# is written to the file traj.log.

cpptraj -p nacl.parm7 -i cpptraj.in > traj.log

# The intermediary files are removed to clean up the analysis directory.
rm succ.txt iters.txt
cd ..
