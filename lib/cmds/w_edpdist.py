...
class WEDPDist(PlottingMixin,CommonOutputMixin,KineticsAnalysisMixin,TransitionAnalysisMixin,
    ...
    def require_cdfs(self):
        ...
        if self.discard_edf_data:
            westpa.rc.pstatus('Discarding existing EDFs')
        ...
        if remaining_pairs:
            westpa.rc.pstatus('Calculating {:d} EDFs...'.format(len(remaining_pairs)))
            self.calc_cdfs(remaining_pairs)
        else:
            westpa.rc.pstatus('All required EDFs present.')        
        ...
        for (ibin,fbin) in bin_pairs:
            ...
            if len(transitions):
                ...
                for iblock, (block_first, block_past_last) in enumerate(self.iter_block_iter()):
                    westpa.rc.pstatus('\r  {:d}->{:d}: {:d} transitions: iterations [{:d},{:d}]'
                ...
                westpa.rc.pstatus('; N_e = {:d}, mean = {:g}, stdev = {:g}, median = {:g}, 95th percentile = {:g}, max = {:g}'
                                .format(len(full_edf),
                                        float(full_edf.mean()),
                                        float(full_edf.std()),
    ...
    def write_edfs(self):
        if not self.edf_output_pattern:
            westpa.rc.pstatus('Not writing any EDFs in text format.')
            return
        else:
            westpa.rc.pstatus('Writing EDFs to text files...')
        ...
        for (ibin, fbin) in self.selected_bin_pair_iter:
            ...
            westpa.rc.pstatus('  {:{mbw}d}->{:<{mbw}d} to {:s}'.format(ibin,fbin,outfile_name,mbw=mbw))
...
''')
    ...
    def plot_evol(self):
        if not self.evol_plot_pattern:
            westpa.rc.pstatus('Not plotting evolution of duration distribution.')
            return
        else:
            westpa.rc.pstatus('Plotting evolution of duration distributions...')
        ...
        for (ibin,fbin) in self.selected_bin_pair_iter:
            ...
            pyplot.clf()
            westpa.rc.pstatus('  {:{mbw}d}->{:<{mbw}d} to {:s}'.format(ibin,fbin,output_filename,mbw=mbw))
...
if wedp.matplotlib_avail:
    ...
    westpa.rc.pstatus('matplotlib not available; not generating plots')
